...
# Assumes the server runs on localhost, port 8080. 
server = '127.0.0.1'
port = 8888

# Points to copies of the files, not served by the web server
dirbase = '../resources/web'
...
def loadPage(fn):
    s = open(f'{dirbase}/{fn}').read()
    return killWS(s)

def loadImage(fn):
    s = open(f'{dirbase}/{fn}', 'rb').read()
    return s

def requestPageOK(p, pn):
    # Assumes 'index.html' should be added if p is not a file
    if not os.path.isfile(f'{dirbase}/{p}'):
        p = os.path.join(p, f'index{fe}')
